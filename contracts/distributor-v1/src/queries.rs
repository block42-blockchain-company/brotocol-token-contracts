...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        epoch_manager_contract: deps.api.addr_humanize(&config.epoch_manager_contract)?.to_string(),
        rewards_contract: deps.api.addr_humanize(&config.rewards_contract)?.to_string(),
        ...
        bonding_contract: deps.api.addr_humanize(&config.bonding_contract)?.to_string(),
